...
def main():
    ...
    if(s3.exists(f's3://{BUCKET}/{PREFIX}/train') is not True):
         raise ValueError(
                f"--dataset_s3_prefix {data_args.dataset_s3_prefix} and not found under --dataset_s3_bucket {data_args.dataset_s3_bucket} '. "
                "Make sure this is properly initiailized"
            )
